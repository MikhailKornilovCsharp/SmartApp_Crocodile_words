        ...
        if (r.type === 'SERVER_ACTION' && r.act?.action_id === 'help') {
            ...
            if (command.type === 'changeword') {
                ...
                if (appeal === 'official') {
                    ...
                    phraseIndex = Math.floor(Math.random() * officialGreets.length);
                    rsp.msg = unOfficialGreets[phraseIndex] + ` Добро пожаловать в приложение «Слова для Крокодила»! Данный смартап предназначен для всем известной игры, где нужно объяснить слово с экрана, используя только мимику, жесты и движения. Здесь можно попросить новое слово, поменять режим и даже вести счёт отгаданных слов!`;
                    rsp.data = command;
                }
