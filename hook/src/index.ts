        ...
        if (r.type === 'SERVER_ACTION' && r.act?.action_id === 'help') {
            ...
            if (command.type === 'changeword') {
                ...
                if (appeal === 'official') {
                    ...
                    rsp.msg = officialGreets[phraseIndex] + ` Добро пожаловать в приложение «Слова для Крокодила»! Данный смартап предназначен для всем известной игры, где вам нужно объяснить слово с экрана, используя только мимику, жесты и движения. Здесь можно попросить новое слово, поменять режим и даже вести счёт отгаданных слов!`;
                    rsp.data = command;
                } else {
                    phraseIndex = Math.floor(Math.random() * unOfficialGreets.length);
                    rsp.msg = unOfficialGreets[phraseIndex] + ` Добро пожаловать в приложение «Слова для Крокодила»! Данный смартап предназначен для всем известной игры, где тебе нужно объяснить слово с экрана, используя только мимику, жесты и движения. Здесь можно попросить новое слово, поменять режим и даже вести счёт отгаданных слов!`;
                    rsp.data = command;
                }
            } else if (command.type === 'smartapp') {
                if (appeal === 'official') {
                    phraseIndex = Math.floor(Math.random() * officialGreets.length);
                    rsp.msg = officialGreets[phraseIndex] + ` Добро пожаловать в приложение «Слова для Крокодила»! Данный смартап предназначен для всем известной игры, где вам нужно объяснить слово с экрана, используя только мимику, жесты и движения. Здесь можно попросить новое слово, поменять режими, и тут даже есть счёт отгаданных слов!`;
                    rsp.data = command;
                } else {
                    phraseIndex = Math.floor(Math.random() * unOfficialGreets.length);
